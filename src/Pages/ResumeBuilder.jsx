...
const ResumeBuilder = () => {
  ...
  const changeResumeVisibility = async () => {
    try {
      ...
      const { data } = await api.put("/api/resumes/" + resumeId, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
// saveResume section
  ...
  try {
    ...
    await api.put("/api/resumes/" + resumeId, formData, {
      headers: { Authorization: `Bearer ${token}` },
    });
            ...
            <div className="relative w-full">
              <div className="absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2">
                {resumeData.public && (
                  <button
                    onClick={handleShare}
                    className="flex items-center gap-2 px-6 py-2 text-xs bg-gradient-to-br from-blue-100
